...
class Node:
    def __init__(self, axis, value, left, right, point_indices):
        self.axis = axis     # 垂直的轴切  s1,s2,,,sn
        self.value = value   # 切的位置 一个值
        self.left = left     # 左边的区间 也是Node
        self.right = right   # 右边的区间 也是Node
        self.point_indices = point_indices  # store points that belong to this partition ,存点的标号
 
    def is_leaf(self):     # is leaf 没必要切了
...
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    if len(point_indices) > leaf_size:  #小于leaf_size就没必要去切了
        # --- get the split position ---   ## 排序，找到中间点，左右各一半
        ...
        root.left = kdtree_recursive_build(root.left,    ## 左边的丢到递归函数
                                           db,
                                           point_indices_sorted[0:middle_right_idx],
                                           axis_round_robin(axis, dim=db.shape[1]),
                                           leaf_size)
        root.right = kdtree_recursive_build(root.right,  ## 右边的也丢到递归函数
                                           db,
...
def kdtree_construction(db_np, leaf_size):
    N, dim = db_np.shape[0], db_np.shape[1]  # 获取数组数量N 和维度dim
...
# Node   db 节点包含的点  result_set: 结果集   query:查询的点
