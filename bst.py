...
def knn_search(root: Node, result_set: KNNResultSet, key):
    ...
    if root.key >= key:  # query point < root.key, search left
        ...
        if knn_search(root.left, result_set, key): # if key!= query, need to go through one subtree.
            return True                # ( knn_search root 里面root是none返回false, key == query时候 worstDist=0，返回true,不然就一直迭代)            
        elif math.fabs(root.key-key) < result_set.worstDist(): ##  May not need to search for the other subtree, depends on worst distance.
